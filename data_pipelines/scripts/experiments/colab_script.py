...
MODEL_NAME = "large-v3"
LANGUAGE = "de"
RANGE_SLICE = "0:1" # Transcribe the first file
DIARISE = True
PREPROCESS = False
TEMPERATURE = 0.0
BEAM_SIZE = 5
...
def transcribe_and_diarise(audio_path, model_name, language, hf_token, diarise, temperature, beam_size, best_of):
    ...
    print("Transcribing...")
    segments, _ = model.transcribe(audio, beam_size=beam_size, language=language, temperature=temperature)
...
audio_dir = str(config.UNPROCESSED_AUDIO_DIR)
output_dir = str(config.TRANSCRIPTIONS_DIR)
...
for filename in files_to_process:
    ...
    result = transcribe_and_diarise(processed_path, MODEL_NAME, LANGUAGE, HF_TOKEN, DIARISE, TEMPERATURE, BEAM_SIZE, BEST_OF)
