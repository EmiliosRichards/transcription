...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # Circuit breaker (guard rails) to avoid costly API calls during infra issues
    parser.add_argument("--cb-db-fail-threshold", type=int, default=3, help="Open circuit after this many consecutive DB failures; skip further API calls")
    parser.add_argument("--cb-b2-fail-threshold", type=int, default=3, help="Open circuit after this many consecutive B2 upload failures; skip further API calls")
    parser.add_argument("--cb-open-action", choices=["skip","exit"], default="skip", help="When circuit opens: skip API calls (default) or exit non-zero")
    ...
    cb_state: dict[str, Any] = {"db_fail": 0, "b2_fail": 0, "open": False}
    cb_lock = threading.Lock()

    def _cb_note_db_failure() -> None:
        try:
            with cb_lock:
                cb_state["db_fail"] = int(cb_state.get("db_fail", 0)) + 1
                if not cb_state["open"] and cb_state["db_fail"] >= int(args.cb_db_fail_threshold):
                    cb_state["open"] = True
        except Exception:
            pass

    def _cb_note_db_success() -> None:
        try:
            with cb_lock:
                cb_state["db_fail"] = 0
        except Exception:
    ...
    def _cb_note_b2_failure() -> None:
        try:
            with cb_lock:
                cb_state["b2_fail"] = int(cb_state.get("b2_fail", 0)) + 1
                if not cb_state["open"] and cb_state["b2_fail"] >= int(args.cb_b2_fail_threshold):
                    cb_state["open"] = True
        except Exception:
            pass

    def _cb_note_b2_success() -> None:
        try:
            with cb_lock:
                cb_state["b2_fail"] = 0
        except Exception:
    ...
    def _cb_is_open() -> bool:
        try:
            with cb_lock:
                return bool(cb_state.get("open", False))
        except Exception:
            return False

    def _cb_maybe_abort_before_api(audio_file_id: Optional[int]) -> Optional[dict]:
        if _cb_is_open():
            msg = "Circuit open: skipping API calls due to consecutive infra failures"
            if args.cb_open_action == "exit":
                print(msg, file=sys.stderr)
                if args.use_claims and (audio_file_id is not None):
                    try:
                        _db_release_claim(db_engine, audio_file_id=audio_file_id)
                    except Exception:
                        pass
                raise SystemExit(2)
            # skip this item gracefully
            if args.use_claims and (audio_file_id is not None):
                try:
                    _db_release_claim(db_engine, audio_file_id=audio_file_id)
                except Exception:
                    pass
            return {"processed": False, "status": "skipped", "error": "circuit_open"}
        return None
    ...
    def process_key(key: str) -> dict:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            err_msg = None
            data = None
            # Circuit breaker: avoid costly API calls if infra failing
            pre = _cb_maybe_abort_before_api(audio_file_id)
            if pre is not None:
                return pre
            try:
