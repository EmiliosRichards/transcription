## Manuav Evaluation Rubric (v4, English — single document)
...
- **Do not hallucinate**. If key data is missing, state it as unknown and be conservative.
...
- **B2B clarity**: If a company is *pure consumer-only* with no plausible B2B wedge, Manuav fit is usually low.
  - IMPORTANT: Mixed B2C/B2B should NOT be penalized just because the company is “mostly B2C”.
  - If there is **any credible B2B procurement / institutional buying wedge** (even a minority segment) where Manuav could build a call list and run outbound (e.g., workwear/PPE for businesses, team/club outfitting, bulk orders/invoice purchasing, wholesale/trade accounts), treat this as **potential**.
  - In mixed cases, the score should be driven mainly by: callable buyer persona clarity + plausibility of deal size/LTV + existence of a repeatable sales motion (quotes, invoices, account management, recurring needs), not by “% of revenue that is B2B”.
...
- Treat as **viable for outbound** if there is any credible evidence of business purchasing and a reachable buyer persona, e.g.:
...
- If the company is **mixed** (some B2B signals but mostly consumer-first), do NOT treat the mix itself as a negative. Reflect uncertainty via **confidence** and via `concerns` (what must be validated).
  - Scoring guidance for mixed commerce:
    - If a B2B wedge exists but the economics/sales motion are unclear: typical outcome is **5–7** (potential exists; validate).
    - If there is explicit B2B motion + higher deal sizes (wholesale, trade accounts, invoices, bulk) and a callable buyer persona (procurement/ops/club board): **6–8** can be justified.
...
Readiness red flag (important for Manuav):
- **Beta/waitlist + no pricing + limited customer proof** often means the company is **not ready for scalable cold outbound yet**.
  - Outbound can still be possible in very early phases, but usually requires a clearer ICP, validated offer, and at least some credible proof points (even a small number of named design partners or measurable outcomes) to make cold calls effective and defensible.
