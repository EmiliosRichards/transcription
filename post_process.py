...
def process_transcription_with_llm(transcription_text, base_filename):
    ...
    # --- Logging ---
    os.makedirs("llm_context", exist_ok=True)
    os.makedirs("llm_response", exist_ok=True)
    with open(f"llm_context/{base_filename}.txt", "w", encoding="utf-8") as f:
        f.write(prompt)
    ...
    try:
        ...
        with open(f"llm_response/{base_filename}.txt", "w", encoding="utf-8") as f:
